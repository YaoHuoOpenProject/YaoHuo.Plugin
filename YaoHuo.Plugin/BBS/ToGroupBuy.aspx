...
<%
    ...
    {
        strhtml.Append("<b>您的RMB数量不足！</b><br/>");
    }
    strhtml.Append("</div>");
    if (this.INFO != "OK" && this.INFO != "CLOSE" && this.INFO != "NOTRIGHT" && this.INFO != "MASTERNO")
    {
        ...
        // 修改价格计算方式
        strhtml.Append("需要RMB <b>￥<span id=\"price\" data-initial-price=\"" + idVo.rank + "\">" + (idVo.rank * int.Parse(numValue)).ToString("F2") + "</span></b><br/>");
        strhtml.Append("<form name=\"f\" action=\"" + http_start + "bbs/togroupbuy.aspx\" method=\"post\">");
        strhtml.Append("输入密码 ");
        strhtml.Append("<input type=\"password\" name=\"changePW\" value=\"\" required /><br />"); // 修改 input 类型为 password
        strhtml.Append("<input type=\"hidden\" name=\"siteid\" value=\"" + siteid + "\" />");
